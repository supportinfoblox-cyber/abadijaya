  ...
  return {
    ...
    configureServer(server: any) {
      server.middlewares.use('/api/otrs/session', (req: any, res: any) => {
        ...
        const py = spawn('python3', [path.resolve(__dirname, 'scripts/otrs_close_bridge.py'), '--check-session']);
        let output = '';
      ...
      server.middlewares.use('/api/otrs/close', (req: any, res: any) => {
        ...
        req.on('end', () => {
          const py = spawn('python3', [path.resolve(__dirname, 'scripts/otrs_close_bridge.py'), '--json-stdin']);
          let output = '';
          let errOutput = '';
          py.stdout.on('data', (chunk: any) => { output += chunk.toString(); });
      ...
      server.middlewares.use('/api/otrs/cache', (req: any, res: any) => {
        const fs = require('fs');
        const cachePath = path.resolve(__dirname, 'scripts/otrs_tickets_cache.json');
        res.setHeader('Content-Type', 'application/json');
